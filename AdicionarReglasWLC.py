...
def main():
	...
	for nombre in archivo:
		...
		net_connect.send_command("config acl rule destination address "+nombre+" "+str(NumRegla)+direccionu)
		print("Regla Agregada: "+str(NumRegla))
		NumRegla+=1
		net_connect.send_command("config acl rule add "+nombre+" "+ str(NumRegla))
		net_connect.send_command("config acl rule action "+nombre+" "+ str(NumRegla) +deny)
		net_connect.send_command("config acl rule destination address "+nombre+" "+str(NumRegla)+direcciond)
		print("Regla Agregada: "+str(NumRegla))
		NumRegla+=1
		net_connect.send_command("config acl rule add "+nombre+" "+ str(NumRegla))
		net_connect.send_command("config acl rule action "+nombre+" "+ str(NumRegla) +deny)
		net_connect.send_command("config acl rule destination address "+nombre+" "+str(NumRegla)+direcciont)
		print("Regla Agregada: "+str(NumRegla))
		NumRegla+=1
		net_connect.send_command("config acl rule add "+nombre+" "+ str(NumRegla))
		net_connect.send_command("config acl rule action "+nombre+" "+ str(NumRegla) +deny)
		net_connect.send_command("config acl rule destination address "+nombre+" "+str(NumRegla)+direccionc)
		print("Regla Agregada: "+str(NumRegla))
